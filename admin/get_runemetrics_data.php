...
if ($getMembersResult->num_rows > 0) {
	while ($member = $getMembersResult->fetch_assoc()) {
		...
		foreach ($monthlyXp as $skillId => $months) {
			...
			elseif ($skillId === 28) {
				foreach ($months as $month => $xp) {
					...
					$totalLevelData[$month] += $level;
					$monthStr = $dates[$month-1];
					$xpTotalsData[] = [$skillId, $monthStr, $level, $virtualLevel, $skillXp];
				}
			}
			else {
				foreach ($months as $month => $skillXp) {
					...
					$totalLevelData[$month] += $level;
					$monthStr = $dates[$month-1];
					$xpTotalsData[] = [$skillId, $monthStr, $level, $skillXp];
				}
			}
		}
		foreach ($monthlyXp as $skillId => $months) {
			if ($skillId === 1) {
				foreach ($months as $month => $skillXp) {
					$monthStr = $dates[$month-1];
					$xpTotalsData[] = [$skillId, $monthStr, $totalLevelData[$month], $skillXp];
				}
			}
			else {
				continue;
		...
		foreach ($xpTotalsData as $key => $value) {
			$xpTotalsInsertQuery = "INSERT INTO stats (user, skill, date, level, xp) VALUES ($memberId, $value[0], $value[1], $value[2], $value[3]);";
